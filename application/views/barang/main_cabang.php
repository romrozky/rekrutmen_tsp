              ...
              <div class="card-body">
                  <div class="row">
					  ...
					  if($this->session->userdata('userlevel')=='1'){
						  ?>
					  <div class="col-3"><button type='button' class='btn btn-info tambah' >Tambah Cabang</button></div>
                          ...
                          <table id='datatableberita' class="table table-bordered table-striped" width='100%'>		
							<thead>
								<tr>
									...
									<th align='center' width='25%'>Nama Cabang</th>
									<th align='center'>Alamat Cabang</th>
						...
						$(document).ready(function() {
						   table = $('#datatableberita').DataTable({
							...
							// Load data for the table's content from an Ajax source
								"ajax": {
								"url": "<?=site_url('Master/ajax_list_cabang')?>",
								"type": "POST"
							},
...
<script>				
	$(document).on('click','.edit',function(){
					$('#tampil_form').load("<?=site_url()?>/Master/form_cabang/"+ $(this).attr('id'),function(){
					$('#modal_form').modal('show');
					});
				});
	$(document).on('click','.tambah',function(){
					$('#tampil_form').load("<?=site_url()?>/Master/form_cabang/",function(){
					$('#modal_form').modal('show');
					});
				});
	$(document).on('click', '.detail', function () {
                location.href = '<?= site_url() ?>/User/detailuser/' + $(this).attr('id');
            });
	$(document).on('click', '.aktifasiuser', function () {
		location.href = '<?= site_url() ?>/User/nonaktif/';
	});
	...
	$(document).on('click','.resetdevice',function(){
		var cnf = confirm("Apakah anda yakin akan mereset device user ini?");
		if (cnf == true) {
			//alert ($(this).attr('id'));
			location.href = '<?=site_url()?>/User/resetdevice/' + $(this).attr('id');
		}				
	});
