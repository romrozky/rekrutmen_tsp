              ...
              <div class="card-body">
                  <div class="row">
					  ...
					  if($this->session->userdata('userlevel')=='1'){
						  ?>
					  <div class="col-3"><button type='button' class='btn btn-info tambah' >Tambah Jenis Barang</button></div>					  
						  <?php
					  }
						...
						$(document).ready(function() {
						   table = $('#datatableberita').DataTable({
							...
							// Load data for the table's content from an Ajax source
								"ajax": {
								"url": "<?=site_url('Barang/ajax_list_jenis')?>",
								"type": "POST"
							},
...
<script>				
	$(document).on('click','.edit',function(){
					$('#tampil_form').load("<?=site_url()?>/Barang/form_jenis/"+ $(this).attr('id'),function(){
					$('#modal_form').modal('show');
					});
				});
	$(document).on('click','.tambah',function(){
					$('#tampil_form').load("<?=site_url()?>/Barang/form_jenis/",function(){
					$('#modal_form').modal('show');
					});
				});
	$(document).on('click', '.detail', function () {
		location.href = '<?= site_url() ?>/Barang/detail_jenis/'  + $(this).attr('id');
	});
